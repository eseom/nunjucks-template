...
import { Token, TokenType, TEMPLATE_KEYWORDS } from './tokens';
...
export class SimpleTokenizer {
  ...
  private scanTagName(): void {
    ...
    if (this.isAlpha(this.current())) {
      const start = this.position;
      while (this.isAlphaNumeric(this.current()) || this.current() === '-' || this.current() === ':') {
        this.advance();
      }
