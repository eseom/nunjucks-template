...
export class HTMLParser {
  ...
  constructor(input: string) {
    ...
    this.tokens = tokenizer.tokenize().filter(token => 
      token.type !== TokenType.WHITESPACE // 공백 토큰 제거 (필요시 유지 가능)
    );
  }
  ...
  public parse(): DocumentNode {
    ...
    while (!this.isAtEnd()) {
      ...
      if (node && (node.type === 'Element' || node.type === 'Text' || node.type === 'Comment' || node.type === 'Doctype' || 'templateType' in node)) {
  ...
  private parseElement(): ElementNode {
    ...
    if (!selfClosing && !isVoid) {
      while (!this.isAtEnd() && !this.isClosingTag(tagName)) {
        ...
        if (child && (child.type === 'Element' || child.type === 'Text' || child.type === 'Comment' || 'templateType' in child)) {
  ...
  private parseAttributes(): AttributeNode[] {
    ...
    while (!this.check(TokenType.TAG_CLOSE) && 
  ...
  private parseText(): TextNode {
    ...
    while (!this.isAtEnd() && 
           (this.check(TokenType.TEXT) || this.check(TokenType.NEWLINE))) {
  ...
  private isClosingTag(tagName: string): boolean {
    return this.check(TokenType.TAG_END_OPEN) && 
  ...
  protected consume(type: TokenType, message: string): Token {
    ...
    throw new Error(`${message} at line ${current.start.line}, column ${current.start.column}. Got ${current.type}`);
  }
}
