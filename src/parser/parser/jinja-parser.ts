...
export class JinjaParser extends HTMLParser {
    ...
    while (!this.check(TokenType.RPAREN) && !this.isAtEnd()) {
      ...
      if (this.check(TokenType.IDENTIFIER)) {
        ...
        if (this.check(TokenType.ASSIGN)) {
          ...
          if (this.check(TokenType.STRING) || this.check(TokenType.NUMBER) || this.check(TokenType.IDENTIFIER)) {
            this.advance(); // 기본값 소비
          }
        }
      }
  ...
  private parseRawStatement(startToken: Token): any {
    ...
    const rawContent = this.tokens.slice(contentStart, this.current)
      .map(t => t.value).join('');
  ...
  private checkTemplateTag(tagName: string): boolean {
    ...
    if (this.current + 1 < this.tokens.length) {
      ...
      // 토큰 타입으로 먼저 확인
      switch (tagName) {
        case 'if': return nextToken.type === TokenType.IF;
        case 'elif': return nextToken.type === TokenType.ELIF;
        case 'else': return nextToken.type === TokenType.ELSE;
        case 'endif': return nextToken.type === TokenType.ENDIF;
        case 'for': return nextToken.type === TokenType.FOR;
        case 'endfor': return nextToken.type === TokenType.ENDFOR;
        case 'block': return nextToken.type === TokenType.BLOCK;
        case 'endblock': return nextToken.type === TokenType.ENDBLOCK;
        case 'extends': return nextToken.type === TokenType.EXTENDS;
        case 'include': return nextToken.type === TokenType.INCLUDE;
        case 'macro': return nextToken.type === TokenType.MACRO;
        case 'endmacro': return nextToken.type === TokenType.ENDMACRO;
        case 'set': return nextToken.type === TokenType.SET;
        default:
          // 일반 식별자로 확인
  ...
  private consumeEndTag(tagName: string): void {
    ...
    switch (tagName) {
      case 'endif': expectedTokenType = TokenType.ENDIF; break;
      case 'endfor': expectedTokenType = TokenType.ENDFOR; break;
      case 'endblock': expectedTokenType = TokenType.ENDBLOCK; break;
      case 'endmacro': expectedTokenType = TokenType.ENDMACRO; break;
      case 'endraw': expectedTokenType = TokenType.IDENTIFIER; break; // endraw는 별도 토큰이 없음
      default: expectedTokenType = TokenType.IDENTIFIER; break;
    }
